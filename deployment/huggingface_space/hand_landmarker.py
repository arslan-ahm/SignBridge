...
(This is a bundled copy of model/hand_landmarker.py for the Hugging Face
Space, which needs a self-contained app.py + its dependencies at this
folder's root -- see deployment/huggingface_space/README.md.)
"""
import os
...
def create_image_detector(max_hands=2, min_detection_confidence=0.5):
    """For processing standalone images (e.g. a dataset of photos), one at a time."""
...
def create_video_detector(max_hands=2, min_detection_confidence=0.5):
    """For processing a live sequence of frames (webcam loop), one call per frame with
    a monotonically increasing timestamp — required by the VIDEO running mode."""
    ensure_model_downloaded()
    options = vision.HandLandmarkerOptions(
        base_options=BaseOptions(model_asset_path=MODEL_PATH),
...
def detect_in_video_frame(detector, rgb_frame, timestamp_ms):
    """Frame-by-frame detection for a live stream. Returns (hand_landmarks_list, handedness_list)."""
    result = detector.detect_for_video(frame_to_mp_image(rgb_frame), timestamp_ms)
    return result.hand_landmarks, result.handedness
